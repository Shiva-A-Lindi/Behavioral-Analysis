...
def set_config_file_openfield(path):
    ...
    cfg = {
        ...
        'reanalyze_existing': False,# bool, True if you wish to reanalyze already analyzed videos 
        'crude_smooth_wind' : 500,# gassian kernel window for crudly smoothing 
        'center_vicinity_h_thresh': 0.1,# peak threshold value for crudly smoothed signal 
        'gauss_window' : 10,# gaussina kernel window for smoothing signal before bandpass filtering
        'bandpass_frequency' : (1, 50),# (low_f, high_f) tuple specifying the freq band to filter for detecting start end of pulse
        'filter_order' : 6,# bandpass filter order
        'start_end_h_thresh': 0.001,# peak threshold value for start end event detection using above specified bandpass filtered sig
        'max_iteration' : 20, 
...
class Pulse :
    ...
    def find_center_vicinities_without_recursion(self, center_vicinity_h_thresh = 0.3, crude_smooth_wind = 100):
        """
    ...
    def determine_start_ends(self, thresh = 0.3):
        """
        
        find the events (peaks in filtered sig) based on the peak heights threshold
        and find start ends of pulses based on that.
    ...
    def pre_process(self,
                    ...
                    filt_order = 10):
        """
        smooth, Filter laser detection signal between the low_f and high_f
        and normalize.
    ...
    def remove_problematic_detections(self):
        ...
        pulse_boundaries = self.find_pulse_boundaries()
        n_to_remove = 0
    ...
    def find_center_vicinities(self, 
        ...
        if not self.enforce_use_laser_detection_only:
            ...
            self.find_center_vicinities_without_recursion(center_vicinity_h_thresh = center_vicinity_h_thresh, 
                                        crude_smooth_wind = crude_smooth_wind)
...
class LongPulse(Pulse):
    ...
    def determine_start_ends(self, thresh = 0.025):
        ...
        n = 1
        pulse_boundaries = self.find_pulse_boundaries()
        diff = np.diff(np.pad(self.signal, (0, n), constant_values = (0,0)),
                       n = n)
        diff = np.roll(self.normalize(diff), n)
        diff[: n] = 0

        positive_deflection = np.where(diff > thresh)[0]
        negative_deflection = np.where(diff < -thresh)[0]
        
        # fig, ax = plt.subplots()
        # ax.plot(self.signal, '-o', ms = 2, lw = 1)
        
        for left, right in zip(pulse_boundaries[:-1], pulse_boundaries[1:]):
            
            ups = positive_deflection[np.logical_and(positive_deflection > left,
                                                     positive_deflection < right)]
            downs = negative_deflection[np.logical_and(negative_deflection > left,
                                                     negative_deflection < right)]

            if len(ups) > 0 and len(downs) > 0: # there should be at least one start and end of a pulse
                
                self.starts = np.append(self.starts, np.min(ups))
                self.ends = np.append(self.ends, np.max(downs))
            #     ax.plot(np.min(ups), 0, 'x', c = 'g')
            #     ax.plot(np.max(downs), 0, 'x', c = 'r')
                
            # ax.axvline(left,0, 10, c = 'g')
            # ax.axvline(right, 0, 5, c = 'r')
...
class SortedExpeiment(Experiment) :
    ...
    def __init__(self, video_filepath, 
        ...
        if not isinstance(inter_stim_interval, str):
            self.inter_stim_interval = round(inter_stim_interval / 1000 * fps)
        else:
            self.inter_stim_interval = inter_stim_interval
            
        if os.path.basename(self.video.dirpath) == 'Video': ## if videos are separated by a folder
            
            self.exp_dir = os.path.dirname( self.video.dirpath  )
        else:
            self.exp_dir = self.video.dirpath
    ...
    def check_if_already_analyzed(self):
        ...
        supposed_analysis_filepath = os.path.join(self.exp_dir, 'Laser', self.result_filename)
    ...
    def find_csv_file(self, folder = 'Laser', d_type = 'analogpulse') :
        ...
        path =  os.path.join( self.exp_dir, folder)
    ...
    def add_file_to_csv(self, filepath, info, file_no, shift_mean = None, shift_sd = None, note = ''):
        
        self.remove_if_already_exists(filepath)
        
        if  shift_mean != None:
            
            data = [self.video.path, info, shift_mean, shift_sd, file_no, self.day_tag, note]
            
        else:
            
            data = [self.video.path, info, file_no, self.day_tag, note]
    ...
    def create_problematic_csv(filepath):
        ...
        if not os.path.exists(filepath):
            ...
            with file:
                # identifying header  
                header = ['path', 'Problem', 'file_no', 'trial', 'note']
    ...
    def create_summary_csv(filepath):
        ...
        if not os.path.exists(filepath):
            ...
            with file:
                # identifying header  
                header = ['path', 'method', 'shift_mean', 'shift_sd', 'file_no', 'trial', 'note']
                writer = csv.DictWriter(file, fieldnames = header)
                writer.writeheader()
